...
function applyConsumableEffect(item) {
    ...
    if (item.effects.health) {
        userData.stats.health = Math.min(100, userData.stats.health + item.effects.health);
    }
    
    if (item.effects.energy) {
        userData.stats.energy = Math.min(100, userData.stats.energy + item.effects.energy);
    }
    
    if (item.effects.intelligence) {
        userData.stats.intelligence += item.effects.intelligence;
    }
    
    if (item.effects.sports) {
        userData.stats.sports += item.effects.sports;
    }
    
    if (item.effects.languages) {
        userData.stats.languages += item.effects.languages;
    }
    
    if (item.effects.creativity) {
        userData.stats.creativity += item.effects.creativity;
    }
    
    // Apply temporary buffs if duration is specified
    if (item.effects.duration) {
        const buffId = `buff_${Date.now()}`;
        userData.buffs.push({
            id: buffId,
            name: item.name || formatItemName(item.id),
...
function updateEquipmentSlot(item, oldItemId = null) {
    ...
    if (oldItemId && oldItemId !== "none") {
        const oldItemData = itemDescriptions[oldItemId];
        if (oldItemData) {
            addItemToInventory({id: oldItemId, type: oldItemData.type, rarity: oldItemData.rarity}, 1);
        }
    }
...
function updateEquipmentDisplay() {
    ...
    const equipmentContainer = document.getElementById('equipmentDisplay');
    if (!equipmentContainer) return;
    
    equipmentContainer.innerHTML = '<h3>Equipped Items</h3>';
    
    // Check if equipment exists
    if (!userData.equipment) {
        equipmentContainer.innerHTML += '<p>No items equipped.</p>';
        return;
    }
    ...
    for (const [slot, itemId] of Object.entries(userData.equipment)) {
        // Skip if not a real equipment or if it's 'none'
        if (slot === 'skin' || itemId === 'none') continue;
        
        const slotElement = document.createElement('div');
        slotElement.className = 'equipment-slot';
        
        const itemInfo = itemDescriptions[itemId];
        if (itemInfo) {
            slotElement.innerHTML = `
                <div class="slot-name">${capitalizeFirstLetter(slot)}</div>
                <div class="item-icon">${getItemIcon(itemId)}</div>
                <div class="item-name">${formatItemName(itemId)}</div>
                <div class="item-effects">${getItemEffectsText(itemInfo.effects)}</div>
            `;
        }
        
        equipGrid.appendChild(slotElement);
    }
...
function getItemIcon(itemId) {
    // Return an emoji icon based on item type
    const iconMap = {
        helmet: '⛑️',
        crown: '👑',
        hat: '🎩',
...
function getItemEffectsText(effects) {
    ...
    for (const [stat, value] of Object.entries(effects)) {
        if (stat !== 'duration') {
            const sign = value > 0 ? '+' : '';
            effectText += `${sign}${value} ${capitalizeFirstLetter(stat)}, `;
        }
    }
    
    return effectText.slice(0, -2); // Remove trailing comma and space
}